...
void display_dashboard(void)
{
    ...
    unsigned short spd = read_adc(0x04)/10.33;
    speed[1] = ((spd%10) + 48);
    speed[0] = ((spd/10) + 48);
    speed[2] = '\0';
    get_time();
    clcd_print(time, (0xC0 + (0)));
    clcd_print(event[index], (0xC0 + (9)));
    clcd_print(speed, (0xC0 + (14)));
}
...
void enter_password(void)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            {
                clcd_print("PASSWORD CRACKED", (0x80 + (0)));
            ...
            else
            {
                ...
                if (no_of_attempts > 0)
                {
                    ...
                    clcd_putch(no_of_attempts + 48, (0xC0 + (0)));
                ...
                else
                {
                    ...
                    clcd_print("Sec Left", (0xC0 + (4)));
                    for (unsigned int j = 6; j--;)
                    for (unsigned int k = 2000; k--;)
                    {
                        clcd_putch(j / 100 + '0', (0xC0 + (0)));
                        clcd_putch((j / 10) % 10 + '0', (0xC0 + (1)));
                        clcd_putch(j % 10 + '0', (0xC0 + (2)));
                    }
                    no_of_attempts = 2;
                    pin = 0;
                    clcd_write(0x01, 0);
...
void display_menu()
{
    unsigned char key2=read_switches(0);
    ...
    clcd_print(menu[m_index], (0x80 + (1)));
    clcd_print(menu[m_index+1], (0xC0 + (1)));
    if(key2 != 0xFF)
    {
        backup=key2;
        wait++;
...
void view_log(void)
{
    ...
    unsigned char key=read_switches(0);
    printlog(i, no);
    if (key == 12)
    {
        kcount++;
...
{
    clcd_write(0x01, 0);
    clcd_print("clear",(0xC0 + (3)));
}
void change_password(void)
{
    clcd_write(0x01, 0);
    clcd_print("change pswrd",(0xC0 + (3)));
}
void set_time(void)
{
    clcd_write(0x01, 0);
    clcd_print("set time",(0xC0 + (3)));
}
